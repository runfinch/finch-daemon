...
// FinchShimNetworking tests that CNI hooks ran correctly via finch-shim by verifying
...
// is wired to finch-shim rather than nerdctl.
func FinchShimNetworking() {
	Describe("finch-shim CNI networking", func() {
		var (
			uClient *http.Client
			version string
			url     string
		...
		It("should assign an IP address to a container on the bridge network, confirming CNI hooks ran", func() {
			...
			// If finch-shim failed to run, the container would have no network interface.
