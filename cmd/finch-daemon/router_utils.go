...
// createNerdctlWrapper creates the Nerdctl wrapper and checks for the finch-shim binary.
func createNerdctlWrapper(clientWrapper *backend.ContainerdClientWrapper, conf *config.Config) (*backend.NerdctlWrapper, error) {
	...
	if _, err := ncWrapper.GetShimBinary(); err != nil {
		return nil, fmt.Errorf("failed to find finch-shim binary: %w", err)
	}
