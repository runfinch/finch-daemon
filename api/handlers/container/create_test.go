...
var _ = Describe("Container Create API ", func() {
	...
	Context("handler", func() {
		...
		Context("translate sysctls", func() {
			It("should return nil for nil input", func() {
				Expect(translateSysctls(nil)).Should(BeNil())
			})

			It("should return empty slice for empty map", func() {
				Expect(translateSysctls(map[string]string{})).Should(BeEmpty())
			})

			It("should handle single sysctl", func() {
				input := map[string]string{
					"net.ipv4.ip_forward": "1",
				}
				expected := []string{"net.ipv4.ip_forward=1"}
				Expect(translateSysctls(input)).Should(Equal(expected))
			})

			It("should handle multiple sysctls", func() {
				input := map[string]string{
					"net.ipv4.ip_forward":          "1",
					"net.ipv6.conf.all.forwarding": "1",
					"kernel.msgmax":                "65536",
				}
				result := translateSysctls(input)
