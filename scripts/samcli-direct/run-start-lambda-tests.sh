...
SAM_DEBUG=1 python -m pytest tests/integration/local/start_lambda -k 'function_timeout or durable' -v --tb=short --log-cli-level=DEBUG 2>&1 | tee start_lambda_output.txt || true
