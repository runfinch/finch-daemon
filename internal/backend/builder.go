...
type NerdctlBuilderSvc interface {
	Build(ctx context.Context, client ContainerdClient, options types.BuilderBuildOptions, buildID string) error
	GetBuildkitHost() (string, error)
}

func (w *NerdctlWrapper) Build(ctx context.Context, client ContainerdClient, options types.BuilderBuildOptions, buildID string) error {
	return w.RunBuild(ctx, client.GetClient(), options, buildID)
}
...
// Following functions are copied from https://github.com/containerd/nerdctl/blob/ff9323859a8d7892d8d72380a17b99395ef9a516/pkg/cmd/builder/build.go.
...
//
//nolint:staticcheck // adding a todo for exposing env in nerdctl library
func (w *NerdctlWrapper) RunBuild(ctx context.Context, client *containerd.Client, options types.BuilderBuildOptions, buildID string) error {
	...
	// #nosec G204 - buildctlBinary is validated by buildkitutil.BuildctlBinary()
