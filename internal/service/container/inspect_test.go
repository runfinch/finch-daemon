...
var _ = Describe("Container Inspect API ", func() {
	...
	Context("service", func() {
		...
		It("should enrich HostConfig with fields from OCI spec and labels", func() {
			inspectWithHostConfig := inspect
			inspectWithHostConfig.HostConfig = &dockercompat.HostConfig{}

			pidsLimit := int64(200)
			specWithFields := &specs.Spec{
				...
				Annotations: map[string]string{
					...
					"nerdctl/hidden": "filtered",
				},
			}
			containerLabels := map[string]string{
				labels.Networks: `["my-net"]`,
			}
			...
			ncClient.EXPECT().InspectContainer(gomock.Any(), con, false).Return(
				&inspectWithHostConfig, nil)
			con.EXPECT().Labels(gomock.Any()).Return(containerLabels, nil)
			con.EXPECT().Spec(gomock.Any()).Return(specWithFields, nil)

			result, err := service.Inspect(ctx, cid, false)
			Expect(err).Should(BeNil())
			Expect(result.HostConfig).ShouldNot(BeNil())
			// CapAdd: CAP_NET_ADMIN is non-default
			Expect(result.HostConfig.CapAdd).Should(ConsistOf("CAP_NET_ADMIN"))
			// CapDrop: 13 defaults missing (CAP_CHOWN is present, so 14-1=13 dropped)
			Expect(result.HostConfig.CapDrop).Should(HaveLen(13))
			Expect(result.HostConfig.PidsLimit).Should(Equal(int64(200)))
			Expect(result.HostConfig.Ulimits).Should(HaveLen(1))
			Expect(result.HostConfig.Ulimits[0].Name).Should(Equal("nofile"))
			Expect(result.HostConfig.Annotations).Should(HaveKeyWithValue("custom-key", "custom-val"))
			Expect(result.HostConfig.Annotations).ShouldNot(HaveKey("nerdctl/hidden"))
			Expect(result.HostConfig.NetworkMode).Should(Equal("my-net"))
		})
	...
	Context("enrichHostConfigFromSpec", func() {
		...
		It("should handle non-nil Process with nil Capabilities", func() {
			hc := &types.ContainerHostConfig{}
			spec := &specs.Spec{
				Process: &specs.Process{
					Capabilities: nil,
			...
			enrichHostConfigFromSpec(hc, spec, nil)
			Expect(hc.CapAdd).Should(BeNil())
			Expect(hc.CapDrop).Should(BeNil())
			// Ulimits should still be extracted
			Expect(hc.Ulimits).Should(HaveLen(1))
		})
		It("should handle nil Linux in spec", func() {
			hc := &types.ContainerHostConfig{}
			spec := &specs.Spec{
				Linux: nil,
			}
			enrichHostConfigFromSpec(hc, spec, nil)
			Expect(hc.PidsLimit).Should(Equal(int64(0)))
		})
		It("should not set Annotations for empty annotations map", func() {
			hc := &types.ContainerHostConfig{}
			spec := &specs.Spec{
				Annotations: map[string]string{},
			}
