...
var _ = Describe("Container Create API ", func() {
	...
	BeforeEach(func() {
		...
		ncExe = "/usr/local/bin/finch-shim"
		image = "test-image"
	...
	Context("service", func() {
		It("should successfully create a container", func() {
			ncContainerSvc.EXPECT().GetShimBinary().Return(ncExe, nil)
		...
		It("should return an error if finch-shim binary was not found", func() {
			mockErr := errors.New("could not find shim binary")
			ncContainerSvc.EXPECT().GetShimBinary().Return("", mockErr)
